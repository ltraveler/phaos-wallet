...
export class IRC20 {
  ...
  @mutateState
  claimFor(for: Address): void{
    Host.createGetIdentityPromise(for, 100000).then("_claim", [], Balance.from(0), 3000000)
  }

  _claim(): void{
    let caller = Context.originalCaller()
    util.assert(Context.epoch() <= 104, "Tokens can no longer be claimed")
    ...
    //let age = 6 -- set age manually for running tests
